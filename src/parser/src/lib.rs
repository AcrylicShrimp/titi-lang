...
fn parse_stmt(parser: &mut Parser<impl Iterator<Item = Token>>) -> Result<Stmt, (String, Span)> {
    if parser.expect_keyword(STRUCT) {
        ...
        parser.expect_begin();
        let kind = if parser.expect_kind(TokenKind::Assign) {
            parser.expect_begin();
            let rhs = parse_expr(parser, true)?;

            StmtKind::Assign(Assign {
                span: expr.span.to(rhs.span),
                lhs: expr,
                rhs,
...
fn parse_expr(
    ...
    parse_expr_binary_log_or(parser, allow_object_literal)
}
