use crate::{
    deduce::lookup_id, MirContext, MirStructFieldKind, MirTy, MirTyDef, MirTyKind, MirTyRefKind,
};
use hir::{GlobalContext, InFnContext, InFnExprDef, InFnExprKind, InFnScopeDef, ScopeRef};
use std::collections::HashMap;

#[derive(Debug, Clone)]
pub struct TypedInFnExpr {
    pub ty: MirTy,
    pub def: InFnExprDef,
}

impl TypedInFnExpr {
    pub fn is_assignable(&self, ctx: &InFnContext) -> bool {
        if let Some(ref_kind) = self.ty.ref_kind {
            if let MirTyRefKind::Mref = ref_kind {
                return true;
            }
        }

        match &ctx.exprs[self.def.0].kind {
            InFnExprKind::Index(..) => true,
            InFnExprKind::Member(..) => true,
            InFnExprKind::Id(id) => {}
            _ => false,
        }
    }
